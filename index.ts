...
import { collectTypeScriptErrors } from "./src/collectors";
import { detectMonorepo } from "./src/detectors";
import { generateTSV } from "./src/generators";
	...
	.action(
		async (directory: string, options: { output: string; quiet: boolean }) => {
			try {
				...
				if (!options.quiet) {
					console.log(chalk.gray("[DIR]") + " Scanning directory: " + chalk.cyan(targetDir));
					console.log(chalk.gray("[OUT]") + " Output file: " + chalk.cyan(outputPath) + "\n");
				}
				...
				if (monorepoInfo.isMonorepo) {
					detectSpinner.succeed(
						chalk.green("Detected ") +
							...
							chalk.green(` monorepo with ${monorepoInfo.packages.length} package(s)`),
					);

					if (!options.quiet && monorepoInfo.packages.length > 0) {
						console.log(chalk.dim("  Packages:"));
						monorepoInfo.packages.forEach((pkg) => {
							console.log(
								chalk.dim("    *") +
									" " +
									chalk.blue(pkg.name) +
				...
				if (errors.length === 0) {
					...
					collectSpinner.warn(
						chalk.yellow(`Found ${chalk.bold(errors.length)} TypeScript error(s)`),
					);
					...
					if (!options.quiet) {
						...
						Object.entries(errorsByPackage).forEach(([pkg, pkgErrors]) => {
